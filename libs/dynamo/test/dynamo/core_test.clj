...
(fact
 (let [inj (di/injector {:database-retriever (fn [chan]
   ...
   (module inj)
   (let [chan-req (di/wait-for inj database-chan)
...
(fact
 (let [inj (di/injector {:dynamodb-config :some-config
                         :dynamodb-get-tables (fn [conf] [])})]
   (module inj)
   (def database-retriever (di/wait-for inj database-retriever))))
...
(do
  ...
  (let [inj (di/injector {:dynamodb-config :config
                          :serve (fn [func reg]
...
(fact
 (let [inj (di/injector {:dynamodb-config :config
                         :dynamodb-get-tables (fn [config]
                                                [config :foo :bar])})]
   (module inj)
   (let [tables (di/wait-for inj database-tables)]
...
(fact
 ...
 (let [chan (async/chan)
       inj (di/injector {:dynamodb-config client-opts
                         :num-database-retriever-threads 1
                         :dynamodb-default-throughput {:read 1 :write 1}
                         :serve (fn [func reg]
                                  ...
                                  (async/close! chan))})]
   (module inj)
   (def req-chan (di/wait-for inj database-chan))
   (async/<!! chan)))
