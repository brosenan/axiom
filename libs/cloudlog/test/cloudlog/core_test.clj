...
We explain writer sets in [our discussion of events](cloudlog-events.html#writers), but for now we can just say the writer set is a piece of
...
When a rule is applied to a fact, it takes ownership over the result (see [events](cloudlog-events.html#writers)).
